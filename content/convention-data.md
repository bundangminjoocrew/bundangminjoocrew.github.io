...
```json
{
  ...
  "rules": {
    ...
    "weightRuleNote": "전략지역 5% 가중치는 대구·경북·경남에만 적용합니다. 강원은 일반지역(1.00)으로 설정했습니다."
  },
  "candidates": {
    "leader": [
      {
...
`weight: 1.05`이므로 원득표율과 별개로 **가중치 적용 전체 누계**가 자동으로 다시 계산됩니다. 강원은 `1.00`, 대구·경북·경남은 `1.05`입니다.
